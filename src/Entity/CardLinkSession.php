...
/**
 * One card-station link/verify attempt — see card-setup.md. Unlike the sentinel-string design this
 * replaced, a row here just holds its final `status` once resolved; nothing needs to be "consumed"
 * by whichever poll reads it first, since resolving (CardService::submitScan()) and reading
 * (CardService::getSessionStatus()) are now two independent, ordinary operations.
 */
...
class CardLinkSession
{
    ...
    /** Verify only: set when the tapped UID resolves to a DIFFERENT known member — see § Privacy in card-setup.md for why this never reaches the station itself. */
    #[ORM\ManyToOne(targetEntity: User::class)]
